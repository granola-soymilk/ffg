...
function setup() {
    ...
    createDiv('Number of Bars:')
        .position(20, 20);
    createSlider(5, 15, 10, 1)
        .position(20, 40)
        .input(() => BARS_X = this.value());
    
    createDiv('Quad Point:')
        .position(200, 20);
    createSlider(0, 40, 0, 1)
        .position(200, 40)
        .input(() => QUAD_POINT = this.value());
    
    createDiv('Bar Spacing:')
        .position(350, 20);
    createSlider(0, 20, 2, 1)
        .position(350, 40)
        .input(() => BAR_SPACING = this.value());
    
    createDiv('Sine Amplitude:')
        .position(500, 20);
    createSlider(0, 1, 0, 0.1)
        .position(500, 40)
        .input(() => SINE_AMPLITUDE = this.value());
    
    createDiv('Height Amplitude:')
        .position(650, 20);
    createSlider(0, 1, 0, 0.1)
        .position(650, 40)
        .input(() => HEIGHT_AMPLITUDE = this.value());
    
    createDiv('Peak Position:')
        .position(800, 20);
    createSlider(0, 1, 0.5, 0.1)
        .position(800, 40)
        .input(() => PEAK_POSITION = this.value());
    
    createDiv('X Offset:')
        .position(950, 20);
    createSlider(-100, 50, 0, 1)
        .position(950, 40)
        .input(() => X_OFFSET = this.value());
}
